<script lang="ts">
	...
	const navItems = [
		...
		{ href: '/settings?tab=maintenance', label: 'Maintenance', icon: 'M11.983 13.943a1.95 1.95 0 100-3.9 1.95 1.95 0 000 3.9z M21 12.983v-1.966l-2.11-.328a7.962 7.962 0 00-.62-1.49l1.27-1.72-1.39-1.39-1.72 1.27a7.962 7.962 0 00-1.49-.62L14.983 3h-1.966l-.328 2.11a7.962 7.962 0 00-1.49.62l-1.72-1.27-1.39 1.39 1.27 1.72c-.25.48-.46.98-.62 1.49L3 11.017v1.966l2.11.328c.16.51.37 1.01.62 1.49l-1.27 1.72 1.39 1.39 1.72-1.27c.48.25.98.46 1.49.62l.328 2.11h1.966l.328-2.11c.51-.16 1.01-.37 1.49-.62l1.72 1.27 1.39-1.39-1.27-1.72c.25-.48.46-.98.62-1.49L21 12.983z' },
	];
		...
		<div class="flex h-full flex-col">
			...
			<nav class="flex-1 space-y-1 px-3 py-4">
				{#each navItems as item}
					<a
						href={item.href}
						data-active={isActive(item.href, $page.url.pathname, $page.url.searchParams) || undefined}
						class="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors
							{isActive(item.href, $page.url.pathname, $page.url.searchParams)
