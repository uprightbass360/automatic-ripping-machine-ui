...
async def test_refresh_populates_cache():
    ...
    with (
        patch("backend.services.system_cache.arm_client.get_system_info", new_callable=AsyncMock, return_value=arm_info),
        patch("backend.services.system_cache.transcoder_client.get_system_info", new_callable=AsyncMock, return_value=transcoder_info),
    ):
...
async def test_refresh_handles_none_from_services():
    ...
    with (
        patch("backend.services.system_cache.arm_client.get_system_info", new_callable=AsyncMock, return_value=None),
        patch("backend.services.system_cache.transcoder_client.get_system_info", new_callable=AsyncMock, return_value=None),
    ):
        await system_cache.refresh()
